package org.hvkz.hvkz.modules.chats.window.messages;
...
class MessageViewHolder extends RecyclerView.ViewHolder
{
    ...
    MessageViewHolder(View itemView, MessagesSelector selector, MessagesListAdapter adapter) {
        super(itemView);
    ...
    void bindMessage(ChatMessage _message) {
    ...
    private void layoutForwardedMessages() {
        if (message.getForwarded() != null) {
            ...
            for (ChatMessage.Forwarded fm: message.getForwarded()) {
                ...
                date.setText(Tools.getDateStringFormat(fm.getTimestamp()));
    ...
    void setupMineDisplay(boolean beforeIsMine) {
    ...
    void setupNotMineDisplay(ChatMessage beforeMessage) {
    ...
    private static class CardWrapper
    {
        ...
        private CardWrapper(View view, User user, Callback<Void> callback) {
            ...
            this.groupName.setText(user.getGroupName());
        ...
        public void onOpenProfileClick(View view) {
            ContextApp.getApp(view.getContext()).getOptionsStorage().isModerated(value -> {
                if (value) {
                    EventChannel.send(new Event<User>(Event.EventType.USER_PROFILE_OPEN).setData(user));
                    dismiss.call(null);
                }
            });
        }
